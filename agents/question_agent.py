...
class QuestioningAgent(object):
    ...
    def build_prompt(
        ...
        prompt = tmpl.format(
            ...
            distractors,
            topic_tail,
            "\n" + inc_samples_ex if inc_samples_ex else "",
        )
...
if __name__ == "__main__":
    ...
    def _normalize_json_candidate(s):
        """Strip prefix/suffix garbage so we can try to parse a JSON object."""
        s = (s or "").strip()
        # Drop "user\nProceed..." or similar prefix; start from first {
        if s and "{" in s:
            start = s.index("{")
            # Optionally trim trailing garbage after last }
            if "}" in s[start:]:
                end = s.rindex("}") + 1
                s = s[start:end]
            else:
    ...
    def _fix_json_and_parse(s):
        """Try to fix common JSON issues and return (parsed_obj, fixed_str) or (None, None)."""
        err_msg = ""
        try:
            return json.loads(s), s
        except json.JSONDecodeError as e:
            err_msg = str(e).lower()
        # Fix 1: unescaped newlines / control characters (collapse all whitespace to space)
        if "control character" in err_msg or "invalid control" in err_msg:
            try:
                fixed = re.sub(r"\s+", " ", s).strip()
                return json.loads(fixed), fixed
            except json.JSONDecodeError:
                pass
        # Fix 2: "Extra data" when first line is "{}" and rest is JavaScript-style (unquoted keys)
        if "extra data" in err_msg and s.strip().startswith("{}"):
            rest = "\n".join(s.strip().split("\n")[1:]).strip()
            if rest:
                # Convert unquoted keys to quoted: "topic": -> "topic":
                try:
                    rest_fixed = re.sub(r'(^|[{,\n])\s*(\w+)\s*:', r'\1"\2":', rest)
                    return json.loads(rest_fixed), rest_fixed
                except json.JSONDecodeError:
                    pass
        return None, None
    ...
    ques = []
    for q in question:
        q = _normalize_json_candidate(q)
        parsed, fixed = _fix_json_and_parse(q)
        if parsed is not None:
            ques.append(parsed)  # append dict, not string
            continue
        try:
            json.loads(q)
        except json.JSONDecodeError as e:
            print(f"Invalid JSON format in question: {q[:300]}...\nError: {e}")
        ...
        out = agent.agent.generate_response(
            prompt.format(q),
        ...
        q = out[0] if isinstance(out, (list, tuple)) and len(out) > 0 else out
        q = _normalize_json_candidate(q if isinstance(q, str) else str(q))
        # Try to parse extractor output into a dict; fall back to string
        try:
